   ...
   <script type="text/javascript" >
	 ...
	 function fn_enable(obj, count){
		 ...
		 for(var i = 1;i<=count;i++){
			 document.getElementById("originalFileName"+i).style.visibility="hidden";
			 document.getElementById("i_imageFileName"+i).disabled=false;
			 document.getElementById("tr_file_upload"+i).style.display="block";
		 }
		 document.getElementById("tr_btn_modify").style.display="block";
		 document.getElementById("tr_btn").style.display="none";
	 }
	 ...
	 function fn_reply_form(isLogOn, url, loginForm){
  ...
  <c:choose> 
  	<c:when test="${not empty imageFileList && imageFileList != 'null'}">
		    ...
		    <tr>
			   ...
			   <td id="originalFileName${status.count }" >
			  ...
			  <tr>
			    <td id="tr_file_upload${status.count }">
  ...
  <tr  id="tr_btn"    >
   <td colspan="2" align="center">
       <c:if test="${member.id == article.id }">
       		<c:choose>
       			<c:when test="${not empty imageFileList && imageFileList != 'null'}">
       				<input type=button value="수정하기" onClick="fn_enable(this.form,${imageFileList.size()})">
       			</c:when>
       			<c:otherwise>
	    ...
	    <c:if test="${member.id != article.id }">
	    	<input type=button value="답글쓰기"  onClick="fn_reply_form('${isLogOn}', '${contextPath}/board/replyForm.do?parentNO=${article.articleNO }','${contextPath}/member/loginForm.do')">
	    </c:if>
